...
function usage() {
	cat <<EOF
保证本节点到其他节点的ssh相关配置
Usage: bash sync_kafka_settings.sh <-f svrs_file | -e svrs_pair>
Tips:
...
update_broker_id_cmd() {
  cat << EOF
    [[ ! -d \$KAFKA_HOME ]] && 'ERROR:请配置环境变量KAFKA_HOME' >&2 && exit 1;
    sed -i 's/^\s*\(broker.id=\).*/\1$1/' \$KAFKA_HOME/config/server.properties;
    exit;
EOF
}
...
for svr in $svrs; do
	...
	for ((i=0; i<${#sync_arr[@]}; i++)); do
		scp ${sync_arr[$i]} root@$svr:${target_arr[$i]} >/dev/null && ssh -n root@$svr $(update_broker_id_cmd $broker_id)
		if (($? == 0)); then
